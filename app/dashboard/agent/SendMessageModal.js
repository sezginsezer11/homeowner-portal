...
import { Send, X, AlertCircle, CheckCircle, TrendingUp, DollarSign, MessageSquare } from 'lucide-react'

const MSG_TYPES = [
  { id: 'value_update', label: 'Value Update', icon: TrendingUp, color: 'text-[#c9a84c]', desc: 'Share market insights & property value news' },
  { id: 'general',      label: 'General',      icon: MessageSquare, color: 'text-[#8fa1ad]', desc: 'General communication with your client' },
]

const TEMPLATES = {
  value_update: {
    subject: 'Your Home Value Update — {month} {year}',
    body: `Hi {name},

I wanted to share some exciting news about the real estate market in your neighborhood.

Based on recent comparable sales, homes like yours in the area are seeing strong demand. The market remains competitive, and your property is well-positioned.

If you'd like a full Comparative Market Analysis (CMA) or are curious about your home's current value, I'd love to connect.
...
export default function SendMessageModal({ recipients, onClose, onSent }) {
  const [type, setType]       = useState('value_update')
  const [subject, setSubject] = useState(TEMPLATES.value_update.subject)
  const [body, setBody]       = useState(TEMPLATES.value_update.body)
      ...
      <div className="bg-[#1a2332] border border-[#344a57]/30 rounded-2xl w-full max-w-xl shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-5">
          ...
          {recipients.length > 1 && (
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {recipients.map(r => (
                  <span key={r.id} className="px-2.5 py-1 bg-[#344a57]/40 text-white text-xs rounded-full">
                    {r.full_name}
                  </span>
                ))}
              </div>
                ...
                return (
                  <button key={t.id} type="button" onClick={() => handleTypeChange(t.id)}
                    className={`p-3 rounded-xl border text-left transition-all ${
                      type === t.id ? 'border-[#c9a84c] bg-[#c9a84c]/10' : 'border-[#344a57]/40 hover:border-[#344a57]'
                    }`}>
          ...
          <div>
            ...
            <p className="text-[#464d4f] text-xs mt-1">{body.length} characters</p>
          </div>
