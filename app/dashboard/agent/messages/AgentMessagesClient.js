...
import { MessageSquare, Send, Edit3, TrendingUp, DollarSign } from 'lucide-react'
import SendMessageModal from '../SendMessageModal'

const TYPE_BADGE = {
  value_update: { label: 'Value Update', cls: 'bg-[#c9a84c]/20 text-[#c9a84c]' },
  rate_alert:   { label: 'Rate Alert',   cls: 'bg-blue-900/30 text-blue-400' },
  general:      { label: 'General',      cls: 'bg-[#344a57]/40 text-[#8fa1ad]' },
}

export default function AgentMessagesClient({ sentMessages, clients }) {
    ...
    <div className="space-y-4 max-w-7xl">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[#8fa1ad] text-sm mt-0.5">{sentMessages.length} sent message{sentMessages.length !== 1 ? 's' : ''}</p>
        </div>
      ...
      {sentMessages.length === 0 ? (
        <div className="bg-[#1a2332] border border-[#344a57]/30 rounded-2xl p-16 text-center">
          ...
          <p className="text-[#8fa1ad] text-sm mt-1 mb-6">Send your first update to a client</p>
          <button onClick={() => handleCompose()}
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <p className="text-[#8fa1ad] text-sm mt-1">
                        To: <span className="text-white">{selected.to?.full_name}</span>
                      </p>
                      <p className="text-[#464d4f] text-xs mt-0.5">{new Date(selected.created_at).toLocaleString()}</p>
                    </div>
                    <button
                      onClick={() => { setComposeRecipients([{ id: selected.to_id, full_name: selected.to?.full_name }]); setShowCompose(true) }}
